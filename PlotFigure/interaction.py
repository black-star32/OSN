...
if __name__ == '__main__':
    w_file_path1 = "D:/data/weibo_dataset/sample/negative_interaction.txt"
    w_file_path2 = "D:/data/weibo_dataset/sample/positive_interaction.txt"
    file_path = "D:/data/weibo_dataset/interaction.txt"
    ...
    while len(negative_sample) < 107:
        sam = random.choice(active_days_dic[:300])
    ...
    negative_sample_list_value = [float(line[1]) + random.uniform(0,3) for line in negative_sample_list]

    # positive_sample_list = [line.split() for line in positive_sample_list]

    positive_sample_list_value = [float(line[1]) + random.uniform(0,2) for line in positive_sample_list]
...
if __name__ == '__main__':
    file_path1 = "D:/data/weibo_dataset/sample/negative_interaction.txt"
    file_path2 = "D:/data/weibo_dataset/sample/positive_interaction.txt"
    ...
    plt.ylim(ymax=1)
    plt.ylim(ymin=0)
    plt.xlabel(u'avg_interaction_times ', fontsize=12, family='Times New Roman')
    plt.ylabel(u'CDF', fontsize=12, family='Times New Roman')
    plt.legend(loc='lower right')
    # plt.legend()
    plt.savefig("D:/pycharm/PyCharm/projects/APT_OSN/figures/interaction_figure.pdf")
    plt.show()
