...
if __name__ == '__main__':
    w_file_path1 = "D:/data/weibo_dataset/sample/negative_similarly.txt"
    w_file_path2 = "D:/data/weibo_dataset/sample/positive_similarly.txt"
    file_path = "D:/data/weibo_dataset/similarly.txt"
    with open(file_path, 'r') as f:
        ...
        for line in lines:
            ...
            dic[line[0]] = line[1]
        print(dic)
        active_days_dic = sorted(dic.items(), key=lambda x: x[1])
        print(active_days_dic)

    negative_sample = set()
    while len(negative_sample) < 87:
        sam = random.choice(active_days_dic[:1000])
        negative_sample.add(sam)
    while len(negative_sample) < 100:
        sam = random.choice(active_days_dic[:5000])
        negative_sample.add(sam)
    while len(negative_sample) < 107:
        sam = random.choice(active_days_dic[4000:5000])
        negative_sample.add(sam)
...
if __name__ == '__main__':
    file_path1 = "D:/data/weibo_dataset/sample/negative_similarly.txt"
    file_path2 = "D:/data/weibo_dataset/sample/positive_similarly.txt"
    ...
    plt.ylim(ymax=1)
    plt.ylim(ymin=0)
    plt.xlabel(u'avg_clu_density', fontsize=12, family='Times New Roman')
